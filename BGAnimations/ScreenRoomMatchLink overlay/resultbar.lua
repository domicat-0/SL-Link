local args = ...
local idx = args[1]
local tag = SL.Global.LinkMatchPlayerList[idx]
local score = args[2]
local pos_x = args[3]
local pos_y = args[4]
...
local af = Def.ActorFrame {
	...
	RefreshCommand = function(self)
		local tag = SL.Global.LinkMatchPlayerList[self:getaux()]
		local pname = SL.Global.LinkMatchPlayerNames[tag]
...
af[#af+1] = Def.Quad {
	...
	RefreshCommand = function(self)
		local tag = SL.Global.LinkMatchPlayerList[self:GetParent():getaux()]
